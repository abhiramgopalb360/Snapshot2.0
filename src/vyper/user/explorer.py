...
class DataProfiler:
    ...
    def create_profile(self,
        ...
        """
        Def: create profiling output of a dataframe

        :param cts_cuts: if want to manually define the cuts please use this param to assign
        :param max_vals: maximum values of a non-numeric variable, if number of values larger than max_val then exclude
        :param max_cats: maximum values of a categorical variable, if number of values larger than max_cats
        then "Categorical+other"
        :param number_of_bins: int, number of bins/breaks of continuous variable
        :param blanks_as_na: bool, replace null with na or not
        :param add_missing_rate: bool, add missing rate calculated column to the profile or not
    ...
    def calculate_weighted_variance(df: pd.DataFrame,
        ...
        :param weight_col: percent is the count of the bucket/total records of the dataset
    ...
    def create_var_profiling_ws(self,
        ...
        for i in input_cols:
            ...
            for j in input_rows:
                ...
                if i != 'B':
                    # current_cell = all_var_profiling_rev_ws[x]
                    current_cell.alignment = Alignment(horizontal='center', vertical='center')
                    # current_cell.border = Border(top = thin, left = thin, right = thin, bottom = thin)
                ...
                if i == 'B':
                    ...
                    if var_prof_df['Variable'].iloc[z-1] != var_prof_df['Variable'].iloc[z-2]:
                        current_cell.border = Border(top=thick, left=thick, right=thin, bottom=thin)
                elif i == 'C':
        ...
        # get number of unique variables in df
        # n = len(unique_var)
        ...
        for i in range(4, max_rows + 4):
            all_var_profiling_ws['D' + str(i)].number_format = '#,###'
            all_var_profiling_ws['E' + str(i)].number_format = '0.00%'
            # all_var_profiling_ws['F' + str(i)].number_format = '#,###'
            all_var_profiling_ws['G' + str(i)].number_format = '#,###'
            all_var_profiling_ws['I' + str(i)].number_format = '#,###'
